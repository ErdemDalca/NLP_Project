...
tfidf_matrix_path = os.path.join(BASE_DIR, "vector_outputs", "tfidf_full_lemma.npz")
tfidf_vectorizer_path = os.path.join(BASE_DIR, "vector_outputs", "tfidf_vectorizer_lemma.pkl")
sentences_path = os.path.join(BASE_DIR, "data", "lemmatized_sentences.csv")
output_path = os.path.join(BASE_DIR, "analysis", "top5_outputs", "top5_sparse_lemma.csv")

# === Giriş cümlesi (lemmatized haliyle)
query = "powerups general every piece control even applicable opponent piece"

# === Verileri yükle
df_sentences = pd.read_csv(sentences_path)
match_index = df_sentences[df_sentences["Lemmatized"] == query].index
...
# === TF-IDF matrisini ve vectorizer'ı yükle
...
# === En benzer 5 cümle (kendisi hariç)
